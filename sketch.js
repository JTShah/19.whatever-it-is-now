...
function setup() {
    ...
    floor=createSprite(512,375,1200,10);
...
function draw() {
    ...
    floor.depth=potion_grp.depth+1;
    floor.depth=tree_grp.depth+1;
    floor.depth=boulder_grp.depth+1;
    ...
    if(gamestate===PLAY) {
        ...
        if(run.y>351) {
            run.changeAnimation("run", run_img);
        }
